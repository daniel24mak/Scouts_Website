...
export async function getStorageSectionData(section) {
  if (section === "inventory") return getSupabaseRows("storage_inventory_summary", "select=id,sku,name,item_kind,unit_name,reorder_level,safety_status,available_quantity,below_reorder_level,is_active&order=name.asc");
  if (section === "assets") return getSupabaseRows("storage_assets", "select=id,item_id,asset_tag,serial_number,status,condition,warranty_expires_on,retired_at&order=asset_tag.asc");
  if (section === "kits") return getSupabaseRows("storage_kits", "select=id,code,name,description,is_active&order=name.asc");
  if (section === "locations") return getSupabaseRows("storage_locations", "select=id,code,name,location_type,parent_id,is_restricted,is_active&order=name.asc");
  ...
  const workflows = {
    ...
    suppliers: ["storage_deliveries", "select=*&order=updated_at.desc&limit=100"],
...
export function recordStorageMovement(payload) {
  return callSupabaseRpc("record_storage_movement", {
    ...
    requested_condition_before: payload.conditionBefore ?? null, requested_condition_after: payload.conditionAfter ?? null,
    requested_borrower_id: payload.borrowerId ?? null, requested_group_id: payload.groupId ?? null,
