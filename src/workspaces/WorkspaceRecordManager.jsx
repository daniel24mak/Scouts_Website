...
export default function WorkspaceRecordManager({ title, noun, rows, fields, onMutate, renderRecord, canManage = true }) {
  ...
  const visibleRows = useMemo(() => {
    ...
    return safeRows
      .filter((record) => !term || Object.values(record).some((value) => ["string", "number"].includes(typeof value) && String(value).toLowerCase().includes(term)))
      .sort((a, b) => String(a.name ?? a.title ?? a.code).localeCompare(String(b.name ?? b.title ?? b.code)));
  }, [query, safeRows]);
  ...
  return <section className="workspace-manager">
    ...
    {safeRows.length ? <label className="workspace-manager-search"><Search size={18} aria-hidden="true" /><span className="sr-only">Search {title.toLowerCase()}</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${title.toLowerCase()}`} /></label> : null}
    {visibleRows.length ? <div className="workspace-manager-list">{visibleRows.map((record) => <article key={record.id}>{renderRecord(record)}{canManage ? <div className="workspace-record-actions"><button type="button" onClick={() => open(record)} title={`Edit ${noun}`}><Edit3 size={16} /></button><button type="button" onClick={() => mutate(record.is_active === false || record.retired_at ? "restore" : "archive", record, {})} title={record.is_active === false || record.retired_at ? "Restore" : "Archive"}>{record.is_active === false || record.retired_at ? <RotateCcw size={16} /> : <Archive size={16} />}</button></div> : null}</article>)}</div> : <div className="workspace-manager-empty"><p>{query ? `No ${title.toLowerCase()} match this search.` : `No ${title.toLowerCase()} yet.`}</p>{canManage && !query ? <button type="button" onClick={() => open()}>Add the first {noun}</button> : null}</div>}
